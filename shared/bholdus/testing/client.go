...
import (
	...
	utils "github.com/ChainSafe/ChainBridge/shared/bholdus"
...
func InitiateNativeTransfer(t *testing.T, client *utils.Client, amount types.U128, recipient []byte, destId msg.ChainId) {
	err := client.InitiateTransferNativeToBridge(amount, recipient, destId)
	if err != nil {
		t.Fatal(err)
	}
}
...
func NewNativeTransferCall(t *testing.T, client *utils.Client, amount types.U128, recipient []byte, destId msg.ChainId) types.Call {
	call, err := client.NewTransferNativeToBridgeCall(amount, recipient, destId)
	if err != nil {
		t.Fatal(err)
	}
	return call
