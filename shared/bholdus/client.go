...
func (c *Client) InitiateTransferNativeToBridge(amount types.U128, recipient []byte, destId msg.ChainId) error {
	log15.Info("Initiating bholdus transfer native to bridge", "amount", amount, "recipient", fmt.Sprintf("%x", recipient), "destId", destId)
	return SubmitTx(c, ChainBridgeTransferNativeToBridgeMethod, types.U8(destId), recipient, amount)
}
...
func (c *Client) NewTransferNativeToBridgeCall(amount types.U128, recipient []byte, destId msg.ChainId) (types.Call, error) {
	return types.NewCall(c.Meta, string(ChainBridgeTransferNativeToBridgeMethod), types.U8(destId), recipient, amount)
}
